...
for epoch in range(args.epochs):
    ...
    for times, (X, labels) in enumerate(train_loader):
        ...
        for d_index in range(args.d_steps):
            ...
            real_L_outputs = D(img_C)
            
            fake_L_inputs = torch.mul(img_A, fake_M_inputs) # new loss
            fake_L_outputs = D(fake_L_inputs)

            # D loss
            # Gradient penalty
            gradient_L_penalty = compute_gradient_penalty(D, img_C, fake_L_inputs)
            # Adversarial loss
    ...
    if epoch % args.image_show_interval == 0:
        ...
        img_list = To_Image(img_list, X_fake)  # 将数据转化为图片 and store all figures in img_list
